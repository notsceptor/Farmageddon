...
# Function that does all the main pathwork
# Generates a path left to right, allowing for movement up and down as well
# If add_loops is true, it will go over the generated path and add the number of
# specified loops in spaces it deems able to - Can be loops on loops as well.
